...
class GeminiRestFruitAnalyzer:
    ...
    def capture_image_from_camera(self):
        ...
        while True:
            ...
            # Enhanced preview
            frame_enhanced = cv2.convertScaleAbs(frame, alpha=1.2, beta=15)
            
            # Guide overlay
            height, width = frame_enhanced.shape[:2]
            center_x, center_y = width // 2, height // 2
            
            cv2.putText(frame_enhanced, "Press SPACE to capture, ESC to exit", 
                       (10, 30), cv2.FONT_HERSHEY_SIMPLEX, 0.8, (0, 255, 0), 2)
            cv2.putText(frame_enhanced, "Position fruit in good lighting", 
                       (10, 60), cv2.FONT_HERSHEY_SIMPLEX, 0.7, (0, 255, 255), 2)
            
            cv2.circle(frame_enhanced, (center_x, center_y), 100, (0, 255, 0), 3)
            cv2.rectangle(frame_enhanced, (center_x-150, center_y-150), 
                         (center_x+150, center_y+150), (0, 255, 0), 2)
            
            cv2.imshow('🍎 Fruit Quality Analyzer - AI', frame_enhanced)
